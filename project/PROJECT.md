...
Changes to agent runtime behavior must include dependency impact analysis across the agent profile, relevant skills, shared rules, guidelines, source registry, supported formats, README usage notes, MCP availability and fallback behavior, output directories, and result-path rules as applicable.

Changes to routing, skill composition, MCP behavior, source handling, output format, fallback behavior, prohibited behavior, or user-facing workflow must be covered by `tests/routing-scenarios.md`. If an existing scenario already covers the change, identify that scenario instead of adding a duplicate.
...
Navigator requirement lookup, ID normalization, MCP availability handling, and raw response normalization are runtime behavior owned by `.github/agent-rules/navigator-mcp.md` and applied from `.github/agents/ba-assistant.agent.md`.
